...
switch (side (_this select 0)) do 
{
	case west:
	{
		// Bluefor Gear
			// Default gear
		_unif = ["NAF_U_GVTV_nfpg"];
		_vest = ["NAF_V_PlateCarrier1_nfpm"];
		_helm = ["NAF_H_Helmet_Spec_nfpg"];
		...
		_goggles = ["rhs_googles_yellow","rhs_googles_clear","rhs_googles_orange"];
			// Vehicle crew gear
		_unifCrew = ["NAF_U_GVTV_marn","NAF_U_GVT_marn","NAF_U_GVTT_marn"];
		...
		_helmJet = ["H_PilotHelmetFighter_O"];
			// Backpacks
		_backpackSmall = ["NAF_AssaultPack_nfpm"];
		_backpackLarge = ["NAF_OperatorBag_nfpm"];
		_backpackMedic = ["NAF_Kitbag_nfpm"];
		_backpackRadio = ["NAF_OperatorBag_nfpm"];
	};
	case east:
	{
		// Opfor Gear
